...
export class AuthService {
  ...
  public async createToken({ email, password }: CredentialsDto): Promise<JwtTokensDto> {
    ...
    await this.userRefreshTokensService.deleteOldRefreshTokens({ owner: { id: user.id } });
  ...
  public async sendResetPassword(data: SendResetPasswordDto): Promise<void> {
    ...
    await this.cacheManager.set(id, code);

    await this.sendMailService.createSendMailJobEvent({
      to: data.email,
